...
\PP{Federated Learning}. Federated Learning (FL) is a decentralized training paradigm designed to learn a global model across \(N\) clients in \(C\) without aggregating their raw data. Let \(w^{(r)}\) denote the model’s parameters after round \(r\). At each round, every client \(i \in C\) computes a local update by minimizing its local loss \(\mathcal{L}_i^{(r)}\bigl(w^{(r)}\bigr)\) over private data, producing an update \(\Delta w_i^{(r)}\). The central server aggregates these local updates to form the new global parameters:
\(
w^{(r+1)} 
\;=\; 
\frac{1}{N} \sum_{i=1}^{N} \Delta w_i^{(r)}.
\)
This iterative process preserves data privacy and minimizes communication overhead by keeping data local while continually refining the global model.
