...
\begin{itemize}[leftmargin=*,itemsep=0.1em, parsep=0em, topsep=0em]
  ...
  \item \textbf{RQ2.} How effective is the categorization-based \gnnshort ensemble and word2vec harmonization scheme, and how does it compare to existing federated learning solutions for handling data heterogeneity?
  \item \textbf{RQ3.} What is the resource consumption of various components of \Sys and its end-to-end processing time on a client machine? 
  \item \textbf{RQ4.} How does \Sys perform in an enterprise-level setting with multiple host machines?
...
We discuss the resource consumption of various components of \Sys in Appendix~\ref{sec:resource_consumption}. Additionally, we present a detailed ablation study in Appendix~\ref{app:ablation} on the following metrics: the effect of the number of \gnnshort\ submodels, federated averaging rounds, differential privacy, and the anomaly threshold on detection performance.
...
\subsection{Comparison with existing FL solutions for heterogeneity.}
\label{sec:fedalternatives}

We conducted experiments to assess the efficacy of existing federated learning solutions for data heterogeneity and the non-IID problem. We evaluated two techniques: FedProx~\cite{li2020federated} and FedOpt~\cite{asad2020fedopt}. FedProx mitigates client heterogeneity by introducing a proximal term to the local loss function, which penalizes deviations from the global model, thereby reducing the impact of statistical heterogeneity. Conversely, FedOpt employs an optimizer on the server side, to aggregate updates from distributed clients, enhancing convergence. We compared these techniques with the traditional FedAvg and the \Sys version of FedAvg, which incorporates semantic harmonization and categorized learning. Table~\ref{fedoptprox} presents the evaluation results. Both FedProx and FedOpt outperform simple FedAvg; however, they do not match the performance of \Sys, which employs novel techniques for addressing heterogeneity in provenance graphs. The reason for this is that when penalizing the deviations between global and local models, the learning focuses only on the patterns common to all clients, neglecting entities unique to each client, leading to high false alarms. This issue is mitigated in our ensemble \gnnshort learning, which is specifically designed to preserve unique patterns across clients.
...
\subsection{Enterprise settings analysis}
\label{cost_metric}
