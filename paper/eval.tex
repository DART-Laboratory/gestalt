...
\subsection{Effect of number of clients on scalability}

\Sys works in a decentralized manner so after deployment the scalability of our system is independent of the number of client machines. Each client will be running \Sys locally after the \fpgl phase is over. However, the number of clients can effect the scalability during the training phase. This can effect the central and utility server. The main task of central server is to perform federated averaging on model weights from clients. This is a simple matrix mean operation and doesnt require much resources even when we have thousands of matrices. The same is the case with utility server. Its task is to sum and average the vectors for overlapping semantic attributes and modern machines are well optimized to perform this task for large number of samples. Moreover, these operations just occur during the training phase and have no impact on scalability once trained system is deployed.
