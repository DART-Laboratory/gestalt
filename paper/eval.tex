...
\item \textbf{RQ3.} What is the effectiveness of \wordvec harmonization in a utility server setting?
...
\item \textbf{RQ6.} How does \Sys operates in an enterprise level setting with multiple host machines?
\item \textbf{RQ7.} How robust is \Sys against adversarial mimicry attacks?
\item \textbf{RQ8.} How does \Sys compare to existing federated learning solutions in handling data heterogeneity?

\end{itemize}

%\wajih {Say here that we have an ablation study on metrics: Effect of Federated Averaging Round, blah blah in Appendix.}

We present a detailed ablation study in Appendix~\ref{app:ablation} on the following metrics: the effect of the number \gnnshort\ submodels, federated averaging rounds, differential privacy, and the anomaly threshold on detection performance. \Fix{Add any missing ablation study metrics here.}
