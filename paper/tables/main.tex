...
\begin{table*}[!t]
  \centering
  \scriptsize
  \caption{Comparison of \Sys with SOTA PIDS. Prec.: Precision; Rec.: Recall; F1.: F1-Score. While \flash performs slightly better, \Sys offers strong privacy and scalability through decentralization. Refer to SOTA PIDS papers for their FP/FN details. The fraction in parentheses for Mirage indicates how many systems (out of the total compared) it outperforms or matches on that metric. \wajih{I asked you to fix the issue not to go back completely. The previous version looks SUPER BAD. No consistency in the column widths. Double check the numbers below as well as I moved columns.} \wajih{Add a sentence about what "-" represents in the caption.}}
  ...
  \begin{tabularx}{\textwidth}{l*{4}{YYY}G{1.4cm}G{1.4cm}G{1.4cm}}
    ...
    E3-TRACE        & 0.99 & 0.99 & 0.99 & \FTP  & \FTR  & \FTF  & --    & --    & --    & 0.81 & 0.40 & 0.54 & \TTP~(\ycmark{1/3}) & \TTR~(\ccmark{3/3}) & \TTF~(\ycmark{1/3}) \\
    E3-THEIA        & 0.98 & 0.99 & 0.99 & \FTHP & \FTHR & \FTHF & \KTHP & \KTHR & \KTHF & 0.25 & 0.05 & 0.08 & \TTHP~(\ycmark{2/4}) & \TTHR~(\ccmark{3/4}) & \TTHF~(\ycmark{2/4}) \\
