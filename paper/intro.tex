...
\wajih{Add MAGIC results in the Appendix and mention in the evaluation section that results related to MAGIC are in the evaluation}
...
\wajih{We also need to handle as ORTHRUS in the related or evaluation section as well. I would love to hear creative way we can handle that.}
...
\wajih{Point out in the paper in the intro or discussion why we don't use DP or our results related to DP.}
...
\wajih{The baseline of each client training their own PISD model is missing. If the performance of each client’s model is high enough then there is no incentive to do FL.}
...
\wajih{highlight in the intro that our principled rejection of DP and TGN for FL in PIDS, based on performance and design constraints.}

\wajih{Can we highlight a contribution that we handle cross host links in our model? If one host connects with another hosts maliciously we can detect that like in the case of lateral movement. This is only possible if we combine hosts logs or interconnect them.}

\wajih{Highlight in the intro that as contribution that we provide formal privacy analysis our system. }

\wajih{Use consistent terminology throughout the paper "categorization-based GNN ensemble" vs "personalized submodel GNN ensemble"}
...
\wajih{In the limitation section, discuss that the utility server still receives encrypted token data from all clients, which could become a throughput bottleneck. So we will explore hierarchical federation techniques [citations] in the future.}
