...
\Sys achieves a 170-fold reduction in network communication costs compared to existing centralized systems. Moreover, the decentralized nature of \Sys allows for much faster inference times, bounded only by the client with the most data, completing the full \optc dataset in a few minutes, whereas existing PIDS, such as \flash and \kairos require several hours.   also provide a detailed analysis of the privacy protection provided by our system in Section~\ref{sec:privacy}. Additionally, we provide a comprehensive analysis of our system's resilience against adversarial attacks in Section~\ref{sec:eval} and Section~\ref{sec:discussion}. \Fix{Aren't all ablation studies in Appendix? If some are in Appendix, we should refer appendix as well.} We provide an extensive ablation study to analyze the efficacy of each component of \Sys in Section~\ref{sec:eval}.
