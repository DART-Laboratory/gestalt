...
% \wajih{Update the following items as we discussed in our meeting.}
\begin{itemize}[topsep=.1ex,itemsep=-.1ex,leftmargin=*]
    \item To the best our knowledge, we are the first to introduce a federated provenance graph learning-based PIDS, \Sys, which is privacy-preserving, highly scalable, and achieves state-of-the-art detection accuracy.
    \item We have introduced a novel \textit{ensemble learning} and \textit{process entity categorization} framework for dealing with diverse heterogeneous client data distributions.
    \item Wwe developed a sophisticated \textit{\wordvec harmonization framework} using a multi-server architecture for secure private aggregation of semantic attributes.
    \item We conduct a comprehensive evaluation of our technique on real-world datasets. The results highlights \Sys's effectiveness in detecting system threats, while being scalable and privacy preserving.
\end{itemize}
