...
\begin{enumerate}[itemsep=0.1em, parsep=0em, topsep=0em]
  ...
  \mati{I think C3 \& C4 are more of design limitations of Kairos and Disdet which complicates applying FL to them. I feel like we should not discuss them as challenges because we dont provide any solutions to them. We just prevented these design choices to avoid these problems.}
  % \labitem{C3}{itm:c3} {\bf Temporal Misalignment.} Aggregating models trained on temporally misaligned data from different clients can result in inconsistencies, impairing the creation of a cohesive global model.
...
\PP{Availability.}  We will open-source our code upon publication to advance privacy-preserving IDS research.
