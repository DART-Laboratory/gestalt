...
\wajih{I wanted to quickly flag something I have been noticing.  There are a lot of small consistency issues in your writing, like figure captions, sentence phrasing, and overall polish. I saw the same in the xPIDS paper too. These things may seem minor, but they really affect how the paper reads. This all comes under the umbrella of "the lack of attention to detail." Every time I have to fix them, it takes away time I could be spending running the lab or focusing on bigger picture stuff. I need you to start caring more about the presentation and quality, not just the content. It reflects on both of us.}
...
\Fix{\noskipheading{5. Formal Privacy Analysis.} \Sys provides formal privacy guarantees to support its dual-server architecture. We prove that (i) model updates sent to the central server provide indistinguishability under dataset perturbations (Central Server Privacy), (ii) the utility server cannot recover original tokens from semantic vectors (Utility Server Privacy), and (iii) malicious clients cannot infer the presence of application-specific tokens from the global model (Client-Level Privacy).}
