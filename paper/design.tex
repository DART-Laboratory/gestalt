...
\begin{figure*}[t!]
  ...
  \caption{High-level architecture of \Sys. In the training phase, our system builds local provenance graphs for each client and trains an ensemble of \gnnshort models. Prior to this, we aggregate semantic models contextually for feature encoding. The local \gnnshort models participate in federated learning to develop a global \gnnshort model, which is then utilized for anomaly detection. Notations are defined in Table~\ref{tab:keynotations}. \wajih{lets use some notations in the Client Side Detection as well. Also, please try to use consistent words in the boxes. }}
  \vspace{-3ex}
  \label{fig:arch}
\end{figure*}
...
\begin{algorithm}[!t]
  ...
  \Output{Harmonized Word2Vec model $\mathcal{E}_{\text{semantic}}$ sent to clients.}
  \BlankLine
  ...
  $\mathcal{E}_{\text{semantic}} \leftarrow$ NewModel($TokenVectors$, $EncryptedTokens$) \tcc*{Constructing a new harmonized model.}
  \ForEach{client $C_i$}{
    Send $\mathcal{E}_{\text{semantic}}$ to $C_i$\\
  }
  \BlankLine
  \Return{Harmonized model $\mathcal{E}_{\text{semantic}}$ has been dispatched to all clients.}\\
  \BlankLine
...
%   \SetKwInOut{Input}{Inputs}
%   \SetKwInOut{Output}{Output}
%   \Input{Number of clients $N$, Number of categories $K_{cat}$, Client datasets $\mathcal{C}$}
%   \Output{Trained global GNN models ${GNN}_{\text{global}}$}
%   \BlankLine
...
%   }
%   \BlankLine
%   \tcc{Categorization of processes into $K_{cat}$ categories.}
...
%       \ForEach{$p \in C_i$}{
%           $j \leftarrow \psi(p)$\\
%           Train GNN submodel on $PGClient_{i}$ using processes in category $j$
%       }
%   }
%   \BlankLine
...
%   }
%   \BlankLine
%   \Return ${GNN}_{\text{global}} \leftarrow \{w_1^{(r)}, w_2^{(r)}, \ldots, w_{K_{cat}}^{(r)}\}$\\
%   \BlankLine
...
% ==================================================
% Algorithm 1: Categorization Process
% ==================================================
\begin{algorithm}[!t]
  \scriptsize
  \DontPrintSemicolon
...
% ==================================================
% Algorithm 2: Submodel Training and Aggregation
% ==================================================
\begin{algorithm}[!t]
  \scriptsize
  \DontPrintSemicolon
