...
\begin{figure*}[t!]
  ...
  \caption{High-level architecture of \Sys. In the training phase, our system builds local provenance graphs for each client and trains an ensemble of \gnnshort models. Prior to this, we aggregate semantic models contextually for feature encoding. The local \gnnshort models participate in federated learning to develop a global \gnnshort model, which is then utilized for anomaly detection. \wajih{Could you add this client icon in this figure where every you say client i, client j, etc.: https://tinyurl.com/icon-host}}
  \vspace{-3ex}
  \label{fig:arch}
\end{figure*}
