...
\begin{algorithm}[!t]
  ...
  \Return $G$\\
  \BlankLine
  \caption{Federated Graph Representation Learning with Differential Privacy}
  \label{alg:federated_learning}
\end{algorithm}
...
Finally, the \textit{Anomaly Detection Module} utilizes the aggregated global model for local anomaly detection on each client machine. Figure \ref{fig:arch} illustrates the overarching architecture of \Sys, with a more comprehensive explanation provided in the following subsections:

\begin{figure}[t!]
  \centering
  \includegraphics[width=0.45\textwidth]{fig/arch.pdf}
  \caption{High Level Architecture of \Sys. \wajih{@mati change this diagram to mirror how the DistDet paper has created the diagram}}
  \vspace{-3ex}
  \label{fig:arch}
\end{figure}
