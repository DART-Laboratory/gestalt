...
\begin{algorithm}[h]
  ...
  \ForEach{encrypted model $M_i'$}{
    \ForEach{token $t$ in $M_i'$}{
      $EncryptedVector \leftarrow M_i'[t]$\\
      \eIf{$TokenVectors$.HasKey($t$)}{
        $TokenVectors[t] \leftarrow TokenVectors[t] + EncryptedVector$\\
        $TokenCounts[t] \leftarrow TokenCounts[t] + 1$\\
      }{
        $TokenVectors[t] \leftarrow EncryptedVector$\\
  ...
  $U' \leftarrow$ Encrypt($TokenVectors$, $E$) \tcc*{Re-encrypt the averaged model.}
  \tcc{Dispatch averaged and encrypted model back to clients.}
  \ForEach{client $C_i$}{
    Send $U'$ to $C_i$\\
  }
  \BlankLine
  \Return{Averaged and encrypted unified model $U'$ has been dispatched to all clients.}\\
  \BlankLine
...
\begin{algorithm}[h]
  ...
  Global GNN model $G$;}
  \Output{Updated global GNN model $G$.}
  \BlankLine
  ...
  \ForEach{client model $M_i$}{
    Send $G$ to $M_i$\\
    $G_i \leftarrow \text{TrainOnLocalData}(M_i)$ \tcc*{Train $G$ locally on $M_i$.}
  }
  \BlankLine
