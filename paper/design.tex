...
Our experiments with the OPTC dataset showed that, on average, different hosts can have up to 75\% overlap in process names, 41\% in files, and 60\% in network flows. This finding aligns with related work \cite{flash2024}, which demonstrates that many system-level processes and files are common across different systems. In centralized PIDS, these attributes are learned by a single semantic encoder, resulting in their mapping to the same embedding space. Conversely, in a federated setting where each client trains its own encoder, model bias may be introduced to the token embeddings, complicating convergence for downstream GNN models using these feature vectors. Therefore, it is crucial to harmonize these overlapping tokens to provide the model with a unified understanding of the semantic information present in system logs. Our system harmonizes only the tokens that overlap in the word2vec models across clients. Tokens that do not overlap are not averaged and remain unchanged. Clients share common activities due to standard system-level routines; however, some variations and patterns are unique to each client. Non-overlapping tokens preserve these unique patterns. To capture these distinct local variations between clients, we have developed a novel ensemble GNN learning framework, detailed in Section \ref{sys:fpgl}, where each submodel specializes in distinct system patterns across clients, enhancing model precision as shown in Section \ref{sec:eval}.
...
The framework employs a central utility server that plays a pivotal role in the categorization process. Clients transmit lists of encrypted process names to the utility server to ensure privacy. The server aggregates these lists and randomly divides them into \(k\) global categories, where \(k\) is a predefined hyperparameter. We explored the impact of \(k\) on detection accuracy through experimentation, as detailed in Appendix~\ref{app:ablation}.

After receiving the categorized lists from the server, clients organize their processes into \(k\) bins based on the assigned categories. This ensures that each bin contains similar processes across different clients. The processes within each bin are then used to generate provenance subgraphs, capturing their interactions with other system objects. These subgraphs serve as training data for an ensemble of GNN models. Each submodel in the ensemble is trained on the subgraphs corresponding to its designated category and participates in federated averaging.
