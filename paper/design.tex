...
The framework employs a central utility server that plays a pivotal role in the categorization process. Clients transmit lists of encrypted process names to the utility server to ensure privacy. The server aggregates these lists into a \emph{global} list of process names and then randomly divides them into $k$ global categories, where $k$ is a predefined hyperparameter. Although this split is random, it is performed \emph{only once} at the global level. Consequently, all occurrences of a given process name are assigned to the same category across every client.
...
Both clients send encrypted lists of these process names to the utility server, which merges them into a single global list of unique names \(\{\texttt{chrome.exe}, \texttt{ssh}, \texttt{mysql}, \texttt{firefox}\}\). The server then randomly assigns each name to one of the $k$ categories, where $k$ is a predefined hyperparameter. For instance, the server might place \(\texttt{chrome.exe}, \texttt{mysql}\) in \(\text{category }1\) and \(\texttt{ssh}, \texttt{firefox}\) in \(\text{category }2\).

This categorization is sent back to both clients. Now \emph{all} occurrences of \texttt{chrome.exe} and \texttt{mysql} fall under category~1 for both clients, while \texttt{ssh} and \texttt{firefox} fall under category~2. In this way, each category holds similar process names across all clients, guaranteeing consistency even though the initial split was random.
