...
\begin{figure}[!t]
  ...
  \caption{Adversarial mimicry attack analysis. \wajih{remove grid from the background.} }
  \label{mimicryattack}
  \vspace{-2ex}
\end{figure}
...
\begin{figure}[!t]
    ...
    \hfill
    \caption{Model poisoning attack analysis. \wajih{remove grid from the background.}}
    \label{fig:poison}
    \vspace{-3ex}
  \end{figure}
...
\begin{figure}[!t]
  ...
  \caption{Effect of differential privacy noise on detection using E3 dataset. Note that we observed similar results on the other datasets. \wajih{remove grid from the background.}}
  \label{epsvsscore}
  \vspace{-2ex}
\end{figure}
